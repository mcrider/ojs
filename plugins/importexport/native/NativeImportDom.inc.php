	...
	function handleAuthorNode(&$journal, &$authorNode, &$issue, &$section, &$article, &$errors) {
		...
		for ($index=0; ($node = $authorNode->getChildByName('competing_interests', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.native.import.error.articleAuthorCompetingInterestsLocaleUnsupported', array('authorFullName' => $author->getLocalizedFullName(), 'articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		for ($index=0; ($node = $authorNode->getChildByName('biography', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.native.import.error.articleAuthorBiographyLocaleUnsupported', array('authorFullName' => $author->getLocalizedFullName(), 'articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
