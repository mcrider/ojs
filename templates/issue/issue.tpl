...
{foreach name=categories from=$publishedArticles item=category key=categoryId}
	{if $categoryId != 0}<h2>{$category.title|escape}</h2>{/if}
	{foreach name=sections from=$category.sections item=section key=sectionId}
		<div{if $categoryId !=0} class="categorySection"{/if}>
