...
class SubmitterDAO extends DAO {
	 ...
	 * @return array Users
	 */
	function &getSubmittersBySubmissionId($submissionId) {
		$submitters = array();

		$result =& $this->retrieve(
			'SELECT u.* FROM users u
			 LEFT JOIN submitters s ON (s.user_id = u.user_id) WHERE s.submission_id =  ?',
			(int) $submissionId
		);
		...
		// Add submitter from article table (for legacy reasons)
	...
	function submitterExists($userId, $submissionId) {
		...
		if(!$returner) {
			// If user not in submitter table, check user id in article table
			$articleDao =& DAORegistry::getDAO('ArticleDAO');
			$article =& $articleDao->getArticle($submissionId);
			$user =& $article->getUser();
			if($user->getId() == $userId) $returner = true;
		}
	...
	/**
	 * Delete submitters by submission.
	 * @param $submissionId int
	 */
